...
export default function Profile() {
    ...
    async function handleDeleteClick(id: number) {
        try {
            ...
            setProjects(exps => exps.filter(exp => exp.id !== id));
        } catch (error) {
        ...
        <section className="flex flex-col items-center justify-center min-h-screen bg-gray-100 px-4">
            ...
            <div className="w-full max-w-2xl space-y-6">
                {projects.length > 0 ? (
                    ...
                    <p className="text-gray-700 text-center py-8">No projects found.</p>
                )}
            </div>
            <button
            ...
            {isCreateOpen && (
                <ProjectModal
                    ...
                    onSubmit={data =>
                        api.post("/projects", data).then(res =>
                            setProjects(exps => [...exps, res.data])
                        )
                    }
                />
            )}
            ...
            {isEditOpen && editData && (
                <ProjectModal
                    ...
                    onSubmit={data =>
                        api.put(`/projects/${data.id}`, data).then(res =>
                            setProjects(exps =>
